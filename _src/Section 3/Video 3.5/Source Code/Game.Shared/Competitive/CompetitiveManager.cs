...
namespace Game.Shared.Competitive
{
    ...
    public class CompetitiveManager
    {
        ...
        /// <summary> The active leaderboards </summary>
        ...
        public void LoadData(String[] achievementIds, String[] leaderboardIds)
        {
            ...
            _Scores = new LeaderboardScore[leaderboardIds.Length];
            for (Int32 i = 0; i < _Scores.Length; i++)
            {
                _Scores[i] = new LeaderboardScore(leaderboardIds[i]);
            }
        }
        ...
        /// <summary> Updates the progress on the leaderboard </summary>
        public void UpdateLeaderboardProgress(Int32 index, Int32 score)
        {
            if (score <= _Scores[index].Score) return;
            _Scores[index].UpdateScore(score);

            if (!_Connected) return;
            _PlatformController.UpdateLeaderboard(_Scores[index]);
        }

        /// <summary> View the leaderboards on the specific platform </summary>
        public void ViewLeaderboards()
        {
            if (!_Connected) return;
            _PlatformController.ViewLeaderboards();
        }
    }
}
