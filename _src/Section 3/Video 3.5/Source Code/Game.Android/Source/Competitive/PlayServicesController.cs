...
namespace Game.Android.Source.Competitive
{
    ...
    public class PlayServicesController : CompetitiveController
    {
        ...
        /// <summary> Updates the score on the leaderboard </summary>
        /// <param name="leaderboardScore"></param>
        public override void UpdateLeaderboard(LeaderboardScore leaderboardScore)
        {
            if (!_GoogleApiClient.IsConnected) return;
            GamesClass.Leaderboards.SubmitScore(_GoogleApiClient, leaderboardScore.Id, leaderboardScore.Score);
        }
        ...
        /// <summary> Views the leaderboards on Google Play Services </summary>
        public override void ViewLeaderboards()
        {
            GameActivity.Instance.StartActivityForResult(GamesClass.Leaderboards.GetAllLeaderboardsIntent(_GoogleApiClient), Constants.LEADERBOARD_REQUEST_CODE);
        }
